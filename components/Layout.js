import Head from 'next/head';
import React, { Component } from 'react';
import Link from 'next/link';

export default class Layout extends Component {
  render() {
    return (
      <div className="SiteWrapper bg-black text-white flex flex-wrap">
        <Head>
          <title>
            {this.props.title ? this.props.title : 'Hello'} | sznm.dev
          </title>
          <link rel='icon' href='/favicon.ico' />
        </Head>
        <header className="w-full text-center sm:text-justify bg-black text-white h-24 sticky top-0 py-2">
          <div className="headerContent self-center h-24">
            <Link href={'/'} >
              <h1 className="text-2xl font-bold">Hello</h1>
            </Link>
            <p className="text-sm">projects by sznm</p>
          </div>
        </header>
        {this.props.children}
        <footer className="text-center text-sm sm:text-justify w-full sticky bottom-0 bg-black py-4">
          <div className="footerContent">
            2020 -{' '}
            <a
              className="underline hover:text-teal-400"
              href='https://agustinusnathaniel.com'
              target='_blank'
              rel='noopener noreferrer'
            >
              AgustinusNathaniel
            </a>
          </div>
        </footer>
      </div>
    );
  }
}
